...
│   (Fast Signal)  │  Trained on 127K comments
└────────┬────────┘
│ ml_prob + risk_bucket
▼
┌─────────────────┐
│  Claude API      │  Policy taxonomy classification
│  (Context Layer) │  Severity + reasoning
└────────┬────────┘
│
▼
┌─────────────────┐
│ Disagreement     │  Flags overblocking / underblocking
│ Detection Layer  │  Routes to human review queue
└─────────────────┘
